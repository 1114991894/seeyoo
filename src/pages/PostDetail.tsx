...
import { Heart, MessageCircle, Share2, ArrowLeft, Send, Bookmark } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useNotifications } from '../contexts/NotificationContext';
import { formatDistanceToNow } from '../utils/format';
import { getSensitiveWordsWarning } from '../utils/sensitiveWords';
...
export default function PostDetail() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (savedFavorites) {
        const favoritePosts = JSON.parse(savedFavorites);
        setIsFavorited(favoritePosts.includes(id));
      }
    }
  }, [id, user]);
  ...
  const handleLike = () => {
    ...
    if (hasLiked) {
      // 取消点赞
      updatedLikes = likedPosts.filter((pid: string) => pid !== id);
    ...
    // 保存用户点赞记录
    localStorage.setItem(`shenxiaoyou_user_likes_${user.id}`, JSON.stringify(updatedLikes));
  ...
  const handleFavorite = () => {
    ...
    // 获取用户收藏记录
    const savedFavorites = localStorage.getItem(`shenxiaoyou_user_favorites_${user.id}`);
    const favoritePosts = savedFavorites ? JSON.parse(savedFavorites) : [];

    if (isFavorited) {
      // 取消收藏
      const updatedFavorites = favoritePosts.filter((pid: string) => pid !== id);
      localStorage.setItem(`shenxiaoyou_user_favorites_${user.id}`, JSON.stringify(updatedFavorites));
  ...
  const handleComment = () => {
    ...
    // 敏感词检测
    const warning = getSensitiveWordsWarning(newComment);
    if (warning) {
      alert(warning);
      return;
    }
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-100">
            ...
            <button
              ...
              className={`flex items-center space-x-1 transition-colors ${
                isFavorited ? 'text-emerald-500' : 'text-gray-500 hover:text-emerald-500'
              }`}
            >
